...
headers = ["Name", "Time", "Total /", "Price"] + [f"game{i}" for i in range(1, 21)]
...
# --- Custom Table Editor with Buttons --- ### <-- MODIFIED
st.header("ตารางก๊วน (กด + / - เพื่อเพิ่มลดลูกแบด)")
df_editable = st.session_state.df.copy()
game_cols = headers[4:24]

for idx, row in df_editable.iterrows():
    st.markdown(f"##### {row['Name']} ({row['Time']})")
    row_cols = st.columns(len(game_cols))
    for i, game_col in enumerate(game_cols):
        current_val = str(row[game_col]) if pd.notnull(row[game_col]) else ""
        with row_cols[i]:
            if st.button("➕", key=f"plus_{idx}_{i}"):
                df_editable.at[idx, game_col] = current_val + "l"
            if st.button("➖", key=f"minus_{idx}_{i}"):
                df_editable.at[idx, game_col] = current_val[:-1] if current_val.endswith("l") else current_val
            st.markdown(f"<div style='text-align:center'>{df_editable.at[idx, game_col]}</div>", unsafe_allow_html=True)
